...
query GetAllProjects {
  buildersProjects {
    items {
      ...
      contractAddress
      slug
    GetProjectWithDetails  description
      website
      image
      createdAt
    }
...
- `totalUsers`: The total count of users staking in each project
- `users.items`: The list of all stakers with their staking details
- `users.totalCount`: The count of stakers returned (may be limited by the `limit` parameter)
...
query GetProjectWithDetails($projectId: String!) {
  buildersProject(id: $projectId) {
    ...
    slug
    description
    users(limit: 100, orderBy: "staked", orderDirection: "desc") {
      items {
        id
        address
        staked
    ...
    events(limit: 100, orderBy: "blockTimestamp", orderDirection: "desc") {
      items {
        id
        userAddress
        eventType
...
**Note:** 
- Users limit: **100** (can be increased up to ~250 if needed)
- Events limit: **100** (can be increased up to ~250 if needed)
- Use `totalCount` in the response to see if there are more results than the limit
- For projects with many users/events, consider pagination or separate queries
**Variables:**
```json
{
  "projectId": "0x1234567890abcdef1234567890abcdef12345678"
...
**Note:** This query returns all projects where the specified address is the admin (creator). It includes:
- Full project details
- Top 10 stakers for each project
- Total count of projects created by this admin
